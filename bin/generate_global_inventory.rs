...
fn collect_artifacts(
    ...
    // Component layout: specs are commonly in `docs/*_SPEC.md`.
    if docs_dir.is_dir() {
    ...
    if reviews_dir.is_dir() {
            ...
            fs::read_dir(&reviews_dir).map_err(|e| format!("read_dir reviews failed: {e}"))?
        {
            ...
            let p = entry.path();
            if p.is_file() && p.extension() == Some(OsStr::new("md")) {
                review_docs.push(rel_path(repo_root, &p)?);
            }
        }
    }
    review_docs.sort();
...
fn main() -> Result<(), String> {
    ...
    lines.push(DOCUMENT_HEADER.trim_end_matches('\n').to_string());
