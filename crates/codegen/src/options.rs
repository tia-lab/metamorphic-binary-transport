...
pub fn extensions(pool: &DescriptorPool) -> Result<MbtExtensions> {
    ...
    Ok(MbtExtensions {
        dictionary_values: extension(pool, "mbt.dictionary_values")?,
        schema_id: extension(pool, "mbt.schema_id")?,
        schema_version: extension(pool, "mbt.schema_version")?,
        transport_name: extension(pool, "mbt.transport_name")?,
        payload_root: extension(pool, "mbt.payload_root")?,
        projection: extension(pool, "mbt.projection")?,
        dictionary: extension(pool, "mbt.dictionary")?,
        bitmask_dictionary: extension(pool, "mbt.bitmask_dictionary")?,
        presence_bit: extension(pool, "mbt.presence_bit")?,
        key_part: extension(pool, "mbt.key_part")?,
        key_order: extension(pool, "mbt.key_order")?,
        const_u16: extension(pool, "mbt.const_u16")?,
        repeated_payload: extension(pool, "mbt.repeated_payload")?,
        raw_string: extension(pool, "mbt.raw_string")?,
        ignored: extension(pool, "mbt.ignored")?,
        projection_group: extension(pool, "mbt.projection_group")?,
        derived_utc_from: extension(pool, "mbt.derived_utc_from")?,
    })
}
