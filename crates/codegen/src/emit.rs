...
pub fn inspect(config: &CodegenConfig) -> Result<String> {
    // Inspect reports model shape and generated line count without writing files.
...
pub fn write(config: &CodegenConfig) -> Result<()> {
    // Write updates the output only when formatted generated bytes changed.
    let output = config
        .out
        .as_ref()
...
pub fn check(config: &CodegenConfig) -> Result<()> {
    // Check regenerates into a temp file, then byte-compares against the checked-in artifact.
    let output = config
        .out
        .as_ref()
        .ok_or_else(|| CodegenError::UnsupportedArgument("--out is required".to_string()))?;
...
pub fn smoke_crate(root: &Path, generated_source: &str) -> Result<PathBuf> {
    // Smoke crates compile generated source in isolation from the main workspace.
...
pub fn format_rust(source: &str) -> Result<String> {
    // rustfmt is part of generation determinism, not a manual post-processing step.
...
pub fn write_if_changed(path: &Path, bytes: &[u8]) -> Result<()> {
    // Avoid touching mtimes when generated bytes are identical.
    if path.exists() {
...
fn generated_unformatted(config: &CodegenConfig, model: &SchemaModel) -> Result<String> {
    // Surface dispatch decides which emitter owns the generated artifact.
