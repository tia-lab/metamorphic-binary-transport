...
use crate::measurement_projection::{MAX_RESPONSE_BYTES, measurement_rows};
...
pub struct SchemaHashes {
    pub mbt_full: u64,
    pub mbt_without_details: u64,
    pub mbt_values_only: u64,
}
...
pub fn measure_report(smoke: bool, command: String) -> BenchResult<CompressionReport> {
    ...
    Ok(CompressionReport {
        ...
        fixture_source: "crates/benches/src/measurement_projection.rs::measurement_rows",
...
pub fn required_lanes() -> &'static [&'static str] {
    &["mbt_full", "mbt_without_details", "mbt_values_only"]
}
...
pub fn sample_report() -> CompressionReport {
    let metric = CompressionMetric {
        ...
        source_schema_hash: MeasurementV1::SCHEMA_HASH,
        byte_equal: true,
    };
    CompressionReport {
        experiment: "mbt_compression",
...
fn source_bytes(row_count: usize) -> BenchResult<Vec<SourceBytes>> {
    let rows = measurement_rows(row_count);
    let full = MeasurementV1::encode(&rows, MAX_RESPONSE_BYTES)?;
    let without_details = MeasurementV1::project_without_details(&full, MAX_RESPONSE_BYTES)?;
    let values_only = MeasurementV1::project_values_only(&full, MAX_RESPONSE_BYTES)?;
    Ok(vec![
        SourceBytes {
            lane: "mbt_full",
            schema_hash: MeasurementV1::SCHEMA_HASH,
            bytes: full,
        },
        SourceBytes {
            lane: "mbt_without_details",
            schema_hash: MeasurementV1WithoutDetails::SCHEMA_HASH,
            bytes: without_details,
        },
        SourceBytes {
            lane: "mbt_values_only",
            schema_hash: MeasurementV1ValuesOnly::SCHEMA_HASH,
            bytes: values_only,
        },
    ])
}
...
fn schema_hashes() -> SchemaHashes {
    SchemaHashes {
        mbt_full: MeasurementV1::SCHEMA_HASH,
        mbt_without_details: MeasurementV1WithoutDetails::SCHEMA_HASH,
        mbt_values_only: MeasurementV1ValuesOnly::SCHEMA_HASH,
    }
}
