...
use crate::measurement_regression::{
    REFERENCE_PARITY_EVIDENCE_GLOB, ROW_COUNTS, measurement_rows, next_measurement_run_path,
    required_current_labels, sample_metadata, sample_row, serde_rows_from_measurement,
    write_report,
};

#[test]
fn next_run_path_refuses_overwrite() -> Result<(), Box<dyn Error>> {
    let root = temp_root("measurement-next-run")?;
    let first = root.join("measurement_regression_run_1.json");
    fs::write(&first, b"existing")?;
    let next = next_measurement_run_path(&root)?;
    if next.file_name().and_then(|name| name.to_str()) != Some("measurement_regression_run_2.json")
    {
        return Err(err("next Measurement run path did not skip existing run"));
    }

    let metadata = sample_metadata(&first);
    let row = sample_row("measurement_mbt_full_encode_inspect_checked", 1);
...
fn required_labels_are_exact() -> Result<(), Box<dyn Error>> {
    ...
    if required_current_labels() != expected {
        return Err(err("Measurement regression labels changed"));
    }
    if ROW_COUNTS != [1, 100, 500, 1_000, 10_000, 100_000] {
        return Err(err("Measurement regression row counts changed"));
    }
    Ok(())
}
...
fn serde_rows_preserve_selected_fields() -> Result<(), Box<dyn Error>> {
    let source = measurement_rows(3);
    let serde_rows = serde_rows_from_measurement(&source);
    ...
    if serde_row.device_ordinal != source_row.device_ordinal {
        return Err(err("device_ordinal was not preserved"));
    }
    if serde_row.interval_ordinal != source_row.interval_ordinal {
        return Err(err("interval_ordinal was not preserved"));
    }
    if serde_row.recorded_at_ms != source_row.recorded_at_ms {
        return Err(err("recorded_at_ms was not preserved"));
    }
    if serde_row.value_d.to_bits() != source_row.value_d.to_bits() {
        return Err(err("value_d was not preserved"));
    }
...
fn report_contains_required_fields() -> Result<(), Box<dyn Error>> {
    let root = temp_root("measurement-report-fields")?;
    let path = root.join("measurement_regression_run_1.json");
...
#[test]
fn metadata_points_to_reference_parity_evidence() -> Result<(), Box<dyn Error>> {
    let root = temp_root("measurement-old-parity-metadata")?;
    let path = root.join("measurement_regression_run_1.json");
    let metadata = sample_metadata(&path);
    let row = sample_row("measurement_mbt_full_encode_inspect_checked", 1);
    write_report(&path, &metadata, &[row])?;
    let text = fs::read_to_string(&path)?;
    if !text.contains(REFERENCE_PARITY_EVIDENCE_GLOB) {
        return Err(err(
