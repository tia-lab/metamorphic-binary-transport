...
pub const GENERATED_SCHEMA_HASH: u64 = 3345754805576751033;
pub const TRANSPORT_NAME: &str = "mbt.test_compatibility.v1";
...
pub fn validate_row(
    ...
    if row.sites_mask & !VALID_SITE_MASK != 0 {
        return Err(TransportError::InvalidBitmask {
            field: "sites",
            value: row.sites_mask,
        });
    }
    ...
    if previous.is_some_and(|prev| {
        (row.tenant_ordinal, row.entity_ordinal, row.recorded_at_ms)
            < (
                prev.tenant_ordinal,
                prev.entity_ordinal,
                prev.recorded_at_ms,
...
pub fn semantic_checksum(rows: &[TestCompatibilityRowV1]) -> u64 {
    let mut checksum = fnv1a64("mbt.test_compatibility.v1".as_bytes());
    for row in rows {
...
pub fn minimal_projection_checksum(rows: &[TestCompatibilityRowV1]) -> u64 {
    let mut checksum = fnv1a64("mbt.test_compatibility.v1-minimal".as_bytes());
    for row in rows {
...
pub fn checksum_row(
    ...
    checksum = update_u64(checksum, row.sites_mask);
...
pub fn checksum_archived_row(
    ...
    checksum = update_u64(checksum, row.sites_mask.to_native());
...
fn inspect_archived_rows(
    ...
    let mut previous = None;
    let mut semantic_checksum = fnv1a64("mbt.test_compatibility.v1".as_bytes());
    let mut minimal_projection_checksum = fnv1a64("mbt.test_compatibility.v1-minimal".as_bytes());
    for archived_row in archived.rows.iter() {
        let row = row_from_archived(archived_row);
        validate_row(&row, previous.as_ref())?;
...
fn row_from_archived(
    ...
    TestCompatibilityRowV1 {
        ...
        sites_mask: row.sites_mask.to_native(),
...
const NO_OPTIONAL_GENERATED_SCHEMA_HASH: u64 = 17088867874628620082;
const NO_OPTIONAL_TRANSPORT_NAME: &str = "mbt.test_compatibility.v1.no_optional";
...
fn no_optional_inspect_archived_rows(
    ...
    let mut previous = None;
    let mut semantic_checksum = fnv1a64("mbt.test_compatibility.v1.no_optional".as_bytes());
...
impl<'a> Iterator for TestCompatibilityV1NoOptionalProjectionRowsIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some(TestCompatibilityV1NoOptionalProjectionRowRef {
            ...
            sites_mask: source.sites_mask.to_native(),
...
const NUMERIC_ONLY_GENERATED_SCHEMA_HASH: u64 = 10470826900078389155;
const NUMERIC_ONLY_TRANSPORT_NAME: &str = "mbt.test_compatibility.v1.numeric_only";
...
fn numeric_only_inspect_archived_rows(
    ...
    let mut previous = None;
    let mut semantic_checksum = fnv1a64("mbt.test_compatibility.v1.numeric_only".as_bytes());
    let mut minimal_projection_checksum = semantic_checksum;
    for archived_row in archived.rows.iter() {
        let row = numeric_only_row_from_archived(archived_row);
        numeric_only_validate_row(&row, previous.as_ref())?;
