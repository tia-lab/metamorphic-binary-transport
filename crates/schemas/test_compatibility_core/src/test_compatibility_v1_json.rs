...
const JSON_FIELD_SITES_MASK: &[u8] = b"\"sites\":";
...
fn write_json_row(
    ...
    write_json_field_prefix(writer, &mut first, JSON_FIELD_SITES_MASK)?;
    write_json_site_bitmask(row.sites_mask.to_native(), writer)?;
...
const NO_OPTIONAL_JSON_FIELD_RECORDED_AT_MS: &[u8] = b"\"recorded_at_ms\":";
const NO_OPTIONAL_JSON_FIELD_STATUS_ORDINAL: &[u8] = b"\"status\":";
const NO_OPTIONAL_JSON_FIELD_SITES_MASK: &[u8] = b"\"sites\":";
...
fn no_optional_write_json_row(
    ...
    writer.string_value(entity_symbol(row.entity_ordinal.to_native())?)?;
    no_optional_write_json_field_prefix(writer, &mut first, NO_OPTIONAL_JSON_FIELD_RECORDED_AT_MS)?;
    writer.i64_value(row.recorded_at_ms.to_native())?;
    no_optional_write_json_field_prefix(writer, &mut first, NO_OPTIONAL_JSON_FIELD_STATUS_ORDINAL)?;
    writer.string_value(status_symbol(row.status_ordinal.to_native())?)?;
    no_optional_write_json_field_prefix(writer, &mut first, NO_OPTIONAL_JSON_FIELD_SITES_MASK)?;
    no_optional_write_json_site_bitmask(row.sites_mask.to_native(), writer)?;
...
fn no_optional_write_json_site_bitmask(mask: u64, writer: &mut JsonWriter) -> Result<()> {
    writer.begin_array()?;
    let mut first = true;
    if mask & (1_u64 << 0) != 0 {
        if first {
...
const NUMERIC_ONLY_JSON_FIELD_RECORDED_AT_MS: &[u8] = b"\"recorded_at_ms\":";
...
fn numeric_only_write_json_row(
    ...
    numeric_only_write_json_field_prefix(
        writer,
        &mut first,
        NUMERIC_ONLY_JSON_FIELD_RECORDED_AT_MS,
    )?;
    writer.i64_value(row.recorded_at_ms.to_native())?;
