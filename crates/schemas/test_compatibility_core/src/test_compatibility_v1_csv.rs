...
fn write_csv_row(
    ...
    writer.comma()?;
    write_csv_site_bitmask(row.sites_mask.to_native(), writer)?;
    writer.comma()?;
...
fn no_optional_write_csv_row(
    ...
    writer.string_cell(status_symbol(row.status_ordinal.to_native())?)?;
    writer.comma()?;
    no_optional_write_csv_site_bitmask(row.sites_mask.to_native(), writer)?;
    writer.comma()?;
    writer.i64_cell(row.required_i64.to_native())?;
    writer.comma()?;
    writer.i32_cell(row.required_i32.to_native())?;
...
fn no_optional_write_csv_site_bitmask(mask: u64, writer: &mut CsvWriter) -> Result<()> {
    writer.begin_array_cell()?;
    let mut first = true;
    if mask & (1_u64 << 0) != 0 {
        if first {
